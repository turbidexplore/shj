...
public interface ShopRepositroy extends JpaRepository<Shop,String> {
    ...
    @Query("select new Shop(s.code,s.companyname,s.logo) from Shop s where  s.status=1  and ( s.brandgroup LIKE CONCAT('%',:brandgroup,'%') or :brandgroup is null ) or  s.status=1  and (s.label LIKE CONCAT('%',:label,'%') or :label is null ) and ( s.brandgroup LIKE CONCAT('%',:brandgroup,'%') or :brandgroup is null ) order by s.create_time asc ")
...
//    @QueryHints(value = { @QueryHint(name = "query", value = "a query for pageable")})
//    @Query("select s from Shop s  where 1=1 :sb ")
//    Page<Shop> recommend(Pageable pageable,@Param("text") String sb);

    @QueryHints(value = { @QueryHint(name = "query", value = "a query for pageable")})
    @Query("select s from Shop s where  s.status=1 and (s.label LIKE CONCAT('%',:text,'%') or :text is null )")
